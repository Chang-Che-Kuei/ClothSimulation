function [ vertsTransformed, X ] = nricp( Source, Target, Options,landS, landT )
...
%     TRT = triangulation(Target.faces, Target.vertices)
%     normalsSource = vertexNormal(TRS);%Source.normals;
%     normalsTarget = vertexNormal(TRT);%Target.normals;
...
% Landmark points
%Front
% landS = [226,350,161,384,142,368,7,272]+1;
% landT = [6091,435,91,394,195,614,879,913]+1;
% Back
% landS = [187,162,200,142,50,4,856,238,304,338,455,346,326,156]+1;
% landT = [221,67,201,152,66,4,851,269,300,391,141,382,374,354]+1;
% Flat
% landS = [226,350,161,384,142,368,7,272,575,560,121,347,380,202]+1;
% landT = [252,312,80,287,166,397,4,299,898,51,62,179,370,809]+1;
% Thin and BigCloth
% landS = [1095,1215,610,358,511,66,502,49,449,0,892,1141,1431,2968]+1;
% landT = [1438,1492,813,218,697,77,681,374,611,0,1207,1480,2066,2839]+1;

%Front
% landS = [824,506,912,347,876,306,602,4]+1;
% landT = [865,544,418,408,904,348,644,4]+1;
% Back
% landS = [271,657,276,741,251,707,4,476]+1;
% landT = [542,850,405,880,349,885,4,660]+1;
% RightArm 
% landS = [47,146,302,14]+1;
% landT = [240,134,405,19]+1;
% LeftArm
DL = D(landS,:);
UL = vertsTarget(landT,:);
...
%     TRS = triangulation(Source.faces, Source.vertices)
%     normalsSource = vertexNormal(TRS);%Source.normals;
%     normalsTemplate =  normalsSource; %Source.normals;
